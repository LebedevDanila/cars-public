...
const pageTitle = `${state.id === null ? 'Создание' : 'Редактирование'} бренда`
...
const fetchData = async () => {
  ...
  const brandReq = await useFetch(`/api/brand/get?id=${state.id}`)
  const brandRes = getResponseData(brandReq, true)
  if (!isSuccessResponse(brandReq, true)) {
    throw createError({
      statusCode: 404,
      statusMessage: brandRes,
    })
  }

  Object.entries(brandRes).forEach(([key, value]) => {
    if (key === 'meta' || key === 'image') {
...
const submit = async () => {
  ...
  const upsertReq = await $fetch('/api/admin/brand/upsert', {
    method: 'POST',
    body: state,
  })
  if (isSuccessResponse(upsertReq)) {
    const upsertRes = getResponseData(upsertReq)
    window.location.href = `/admin/brand/${upsertRes.id}`
  }
}
...
<template>
  <div class="admin-content">
    ...
    <div class="admin-field __COLUMN">
      <label>Статус</label>
      <div class="admin-checkbox">
        <input v-model="state.status" type="checkbox" />
        <span>{{ state.status ? 'Включен' : 'Выключен' }}</span>
      </div>
    </div>
